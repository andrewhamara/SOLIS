...
model = SOLIS(embed_dim=1280, ff_dim=3072, num_heads=20, num_layers=8).to(DEVICE)
...
PGN_SAVE_PATH = f'/data/hamaraa/solis_big_d{DEPTH}_w{WIDTH}_allratings_vs_stockfish.pgn'

if small:
    #model.load_state_dict(torch.load("/data/hamaraa/solis_latest_small.pth", map_location=DEVICE))
    model.load_state_dict(torch.load("/data/hamaraa/solis_small_stepstep=200000.ckpt", map_location=DEVICE)["state_dict"])
    ...
    model.load_state_dict(torch.load("/data/hamaraa/solis_big_stepstep=40000.ckpt", map_location=DEVICE)["state_dict"])
    v_white = np.load("/data/hamaraa/mean_white_checkmate_big.npy")
    v_black = np.load("/data/hamaraa/mean_black_checkmate_big.npy")
