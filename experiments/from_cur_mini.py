...
@torch.no_grad()
def score_positions(boards, parent_board=None):
    """
    return list[float] of (z' - z_parent) · (μ_white - μ_black),
    with Zobrist+LRU cache
    """
    # if no parent provided, default to the first board's parent position
    if parent_board is None:
        raise ValueError("parent_board must be provided for relative-to-current scoring")

    # embed parent once
    parent_emb = embed_fens([parent_board.fen()])[0]  # [D]
    ...
    with torch.cuda.amp.autocast(enabled=(DEVICE.type == "cuda")):
        emb = model(batch)  # [M, D]
        deltas = emb - parent_emb.unsqueeze(0)
        scores = torch.matmul(deltas, v_advantage)
...
def latent_beam_search_policy(board, beam_width=3, depth=3):
    ...
    def search(b, d, player_max):
        ...
        child_scores = score_positions([cb for _, cb in children], parent_board=b)
    ...
    scores = score_positions([cb for _, cb in children], parent_board=board)
