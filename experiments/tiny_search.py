...
model = SOLIS(embed_dim=256, ff_dim=512, num_layers=6, num_heads=8).to(DEVICE)
...
PGN_SAVE_PATH = f'/data/hamaraa/solis_tiny_d{DEPTH}_w{WIDTH}_allratings_vs_stockfish.pgn'

if tiny:
    #model.load_state_dict(torch.load("/data/hamaraa/solis_latest_small.pth", map_location=DEVICE))
    model.load_state_dict(torch.load("/data/hamaraa/solis_tiny_stepstep=370000.ckpt", map_location=DEVICE)["state_dict"])
    v_white = np.load("/data/hamaraa/mean_white_checkmate_tiny.npy")
    v_black = np.load("/data/hamaraa/mean_black_checkmate_tiny.npy")
else:
