...
def get_dataloader(h5_path, batch_size=512, shuffle=True, num_workers=32, split='train', k_pos=5):
    train_idx, val_idx = split_data(h5_path)
    if split == 'train':
        dataset = SolisDataset(h5_path, k_pos, indices=train_idx)
    else:
        dataset = SolisDataset(h5_path, k_pos, indices=val_idx)
