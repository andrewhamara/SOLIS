...
ax.set_title("3D UMAP Projection of SOLIS Embeddings")
ax.view_init(elev=20, azim=135)  # adjust for nice view angle
plt.tight_layout()
plt.savefig("solis_umap_3d.pdf")
plt.close()
...
def plot_game_on_umap(base_points, traj_points_2d, save_path):
    ...
    # background cloud
    ax.scatter(base_points[:, 0], base_points[:, 1], s=1, alpha=0.15, color="gray")
